...
var FileWidget = SearchableMediaWidget.extend({
        ...
        return this.search('').then(async () => {
            ...
            if (o.url) {
                self._selectAttachement(_.find(self.attachments, function (attachment) {
                    const objectUrl = new URL(o.url, window.location.origin);
                    const attachmentUrl = new URL(attachment.image_src, window.location.origin);
                    // Ignore search params for url matching (such as cachebuster)
                    return (objectUrl.origin + objectUrl.pathname) === (attachmentUrl.origin + attachmentUrl.pathname);
                }) || o);
            }
            return def;
        });
    ...
    _openImageOptimizeDialog: function (attachment, isExisting, $attachmentCell) {
        ...
        var always = () => {
            ...
            const img = this.$media[0];
            const src = new URL(img.src);
            src.searchParams.set('cachebuster', Date.now());
            img.src = src.href;
            img.dataset.removeCacheBuster = 'true';
            const cellImg = $attachmentCell.find('img')[0];
            const cellSrc = new URL(cellImg.src);
            cellSrc.searchParams.set('cachebuster', Date.now());
            cellImg.src = cellSrc.href;
        };
    ...
    _save: async function () {
        ...
        if (img.image_src) {
            ...
            src = new URL(src, window.location.origin);
            src.searchParams.set('cachebuster', Date.now());
            this.$media.attr('src', src.href);
            this.$media[0].dataset.removeCacheBuster = 'true';
        } else {
            if (!this.$media.is('a')) {
